                        ...
                        <ul class="space-y-3">
                            @forelse ($myLowongans as $lowongan)
                                <li wire:key="lowongan-{{ $lowongan->id }}">
                                    <button wire:click="viewApplicants({{ $lowongan->id }})"
                                        class="w-full text-left p-4 rounded-lg transition-colors {{ $selectedLowongan && $selectedLowongan->id == $lowongan->id ? 'bg-indigo-100' : 'hover:bg-gray-100' }}">
                            ...
                            @empty
                                <li class="text-center text-gray-500 py-4">
                                    ...
                                    <a href="{{ route('lowongan.create') }}" wire:navigate
                                        class="text-indigo-600 hover:underline mt-2 inline-block">Publish sekarang</a>
                                </li>
                            @endforelse
                        </ul>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-md">
                        @if ($selectedLowongan)
                            <div class="p-6 border-b border-gray-200">
                                ...
                                <p class="text-xl font-semibold text-indigo-600">{{ $selectedLowongan->judul_lowongan }}
                                </p>
                            </div>
                            <div class="overflow-x-auto">
                                <table class="min-w-full">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="3" class="px-6 py-12 text-center text-gray-500">Belum
                                                    ada pelamar untuk lowongan ini.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
